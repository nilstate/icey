...
{
    openVideo(device, width, height, framerate, pixelFmt);
}
                             ...
                             double framerate, const std::string& pixelFmt)
{
    ...
    // A pixel format must be passed or the capture may error out with:
    // [mjpeg @ 0x...] Specified pixel format -1 is invalid or not supported
    std::string pixfmt = pixelFmt.empty() ? "yuv420p" : pixelFmt;

    // Set custom parameters for the device
    if (width > 0 && height > 0)
    ...
    av_dict_set(&iparams, "pixel_format", pixfmt.data(), 0);
    ...
    // perform pixel conversions and resizing ourselves (on the decoder).
    if (_video) {
        if (!pixfmt.empty())
            _video->oparams.pixelFmt = pixfmt;
        if (width > 0)
