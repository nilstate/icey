...
void Authenticator::fromUserInfo(std::string_view userInfo)
{
    std::string username;
    std::string password;
...
void Authenticator::authenticate(http::Request& request, const http::Response& response)
{
    for (const auto& [name, value] : response) {
        if (util::icompare(name, "WWW-Authenticate") == 0 && isBasicCredentials(value)) {
...
{
    for (const auto& [name, value] : response) {
        if (util::icompare(name, "Proxy-Authenticate") == 0 && isBasicCredentials(value)) {
...
{
    const auto p = userInfo.find(':');

    if (p != std::string_view::npos) {
        username.assign(userInfo.data(), p);
        password.assign(userInfo.data() + p + 1, userInfo.size() - p - 1);
    } else {
        username.assign(userInfo.data(), userInfo.size());
        password.clear();
    }
}
                        ...
                        std::string& password)
{
    if (!uri.userInfo().empty()) {
        extractCredentials(std::string_view(uri.userInfo()), username, password);
    }
}
