...
All types live in the `icy::crypto` namespace. Headers are under `icy/crypto/`. Link against the `icy_crypto` CMake target:

```cmake
target_link_libraries(my_target PRIVATE icy_crypto)
```
